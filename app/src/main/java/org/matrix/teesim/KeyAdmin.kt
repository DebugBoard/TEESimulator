 ...
 * (removes those keyentry ids from keystore2, marker- and target-verified) GET
 * /keys/inspect?alias=A -> { ok, alias, attestation{...} | null } POST /keys/delete?alias=A -> {
 * ok, deleted } GET /logs?after=N&max=M -> { ok, lines:[{seq,level,tag,text}], nextAfter } GET
